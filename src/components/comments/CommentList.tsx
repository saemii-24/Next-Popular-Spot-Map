...
const CommentList = ({ comments, displayStore }: CommentListProps) => {
        ...
        comments?.data?.map((comment) => (
          ...
          >
            ...
            <div className="flex flex-col space-y-1 flex-1">
              <div>{comment?.user?.email ?? "사용자"}</div>
              ...
              {displayStore && comment.store && (
                <div className="mt-2">
                  <Link
                    ...
                    className="text-blue-700 hover:text-blue-600 text-xs underline font-medium"
                  >
                    {comment.store.name}
                  </Link>
                </div>
